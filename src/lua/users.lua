...
-- $Id: users.lua 2557 2013-09-16 15:08:08Z kk $
-- ---------------------------------------------------------------------------
...
function delUser(user)
  ...
  if LC.sys.user_exists(user) then
    ...
    if LC.fs.is_file('/usr/bin/killall') then
      os.execute('/usr/bin/killall -s KILL -u ' .. user)
    end
...
--  if LC.hooks then
--    LC.hooks.check("LC.users.chShell", user, group, home, shell)
--  end
